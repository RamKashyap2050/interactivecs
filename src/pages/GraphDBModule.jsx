...
import { graphDBModuleData } from '../data/graphDBModuleData';
import GraphModelAnim from '../animations/databases/graphdb/GraphModelAnim';
import GraphRepAnim from '../animations/databases/graphdb/GraphRepAnim';
import GraphTraversalAnim from '../animations/databases/graphdb/GraphTraversalAnim';
import CypherAnim from '../animations/databases/graphdb/CypherAnim';
import GraphScaleAnim from '../animations/databases/graphdb/GraphScaleAnim';
...
const GraphDBModule = () => {
  ...
  const chapter = graphDBModuleData.chapters[activeChapterIdx];
  ...
  const renderAnimation = () => {
    const props = { currentStep, isPlaying, speed };
    switch (chapter.animationComponent) {
      case 'GraphModelAnim': return <GraphModelAnim {...props} />;
      case 'GraphRepAnim': return <GraphRepAnim {...props} />;
      case 'GraphTraversalAnim': return <GraphTraversalAnim {...props} />;
      case 'CypherAnim': return <CypherAnim {...props} />;
      case 'GraphScaleAnim': return <GraphScaleAnim {...props} />;
      default: return <div className="text-white p-8">{chapter.animationComponent}</div>;
    }
  };

  const togglePlay = () => setIsPlaying(!isPlaying);
  const stepForward = () => { if (currentStep < totalSteps - 1) { setCurrentStep(p => p + 1); setIsPlaying(false); } };
  const stepBackward = () => { if (currentStep > 0) { setCurrentStep(p => p - 1); setIsPlaying(false); } };
  const restart = () => { setCurrentStep(0); setIsPlaying(true); };

  return (
    <div className="flex h-screen pt-16 bg-[#020611] font-source text-white overflow-hidden">
      {/* Sidebar */}
      <ChapterNav
        chapters={graphDBModuleData.chapters}
        activeChapter={activeChapterIdx}
        onSelect={handleChapterChange}
        color={graphDBModuleData.color}
      />
      ...
      <div className="flex-1 flex flex-col relative h-full">
        ...
        <div className="h-14 border-b border-gray-800 bg-[#0a1526] flex items-center justify-between px-6 z-20 shrink-0">
          <div className="flex items-center gap-3">
            <h2 className="font-space font-bold text-lg" style={{ color: graphDBModuleData.color }}>
              {chapter.title}
            </h2>
            {chapter.bookRef && (
              <div className="flex items-center gap-1.5 text-xs font-mono bg-gray-800/50 text-gray-400 px-3 py-1 rounded-full border border-gray-700">
                ...
                <span className="truncate max-w-[360px]">{chapter.bookRef}</span>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 relative overflow-hidden bg-[#020611]">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <h4 className="text-lg font-space font-bold mb-2" style={{ color: graphDBModuleData.color }}>
          ...
          <div className="flex-grow max-w-2xl flex items-center justify-between mx-12">
            {chapter.steps.map((step, idx) => (
              <div key={idx} className="flex-1 flex items-center">
                ...
                >
                  <div className={`h-1.5 w-full rounded-full transition-all duration-300 ${idx <= currentStep ? 'bg-[#8b5cf6]' : 'bg-gray-800'}`} />
                  <div className={`absolute w-4 h-4 rounded-full transition-all duration-300 ${idx === currentStep ? 'scale-150 ring-4 ring-[#0a1526]' : 'scale-100'} ${idx <= currentStep ? 'bg-[#8b5cf6]' : 'bg-gray-700'}`} />
