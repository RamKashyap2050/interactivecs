...
import { nosqlModuleData } from '../data/nosqlModuleData';
import DocumentModelAnim from '../animations/databases/nosql/DocumentModelAnim';
import KeyValueAnim from '../animations/databases/nosql/KeyValueAnim';
import WideColumnAnim from '../animations/databases/nosql/WideColumnAnim';
import ShardingAnim from '../animations/databases/nosql/ShardingAnim';
import ReplicationAnim from '../animations/databases/nosql/ReplicationAnim';
...
const NoSQLModule = () => {
  ...
  const chapter = nosqlModuleData.chapters[activeChapterIdx];
  ...
  const renderAnimation = () => {
    ...
    switch (chapter.animationComponent) {
      case 'DocumentModelAnim': return <DocumentModelAnim {...props} />;
      case 'KeyValueAnim': return <KeyValueAnim {...props} />;
      case 'WideColumnAnim': return <WideColumnAnim {...props} />;
      case 'ShardingAnim': return <ShardingAnim {...props} />;
      case 'ReplicationAnim': return <ReplicationAnim {...props} />;
      default: return <div className="text-white p-8">Animation: {chapter.animationComponent}</div>;
    }
  };
  ...
  return (
    <div className="flex h-screen pt-16 bg-[#020611] font-source text-white overflow-hidden">
      {/* Sidebar */}
      <ChapterNav
        chapters={nosqlModuleData.chapters}
        activeChapter={activeChapterIdx}
        onSelect={handleChapterChange}
        color={nosqlModuleData.color}
      />
      ...
      <div className="flex-1 flex flex-col relative h-full">
        ...
        <div className="h-14 border-b border-gray-800 bg-[#0a1526] flex items-center justify-between px-6 z-20 shrink-0">
          <div className="flex items-center gap-3">
            <h2 className="font-space font-bold text-lg" style={{ color: nosqlModuleData.color }}>
              {chapter.title}
            </h2>
            {chapter.bookRef && (
        ...
        <div className="flex-1 relative overflow-hidden bg-[#020611]">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <h4 className="text-lg font-space font-bold mb-2" style={{ color: nosqlModuleData.color }}>
          ...
          <div className="flex-grow max-w-2xl flex items-center justify-between mx-12">
            {chapter.steps.map((step, idx) => (
              <div key={idx} className="flex-1 flex items-center">
                ...
                >
                  <div className={`h-1.5 w-full rounded-full transition-all duration-300 ${idx <= currentStep ? 'bg-[#06b6d4]' : 'bg-gray-800'}`} />
                  <div className={`absolute w-4 h-4 rounded-full transition-all duration-300 ${idx === currentStep ? 'scale-150 ring-4 ring-[#0a1526]' : 'scale-100'} ${idx <= currentStep ? 'bg-[#06b6d4]' : 'bg-gray-700'}`} />
